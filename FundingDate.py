...
st.title('Mortgage Application Form for FundingDate Prediction')

# Define the fields for the form
fields = [    'DocSentDate', 'InitialDisclosureDate', 'ApplicationReceivedDate', 'PropertyZip', 'CashoutToBorrower',
    'DocOrderedDate', 'propertyCounty', 'CreateDate', 'Points', 'LockExpirationDate',
    'CostCenter', 'APR', 'ReadyforDocs', 'AppraisalValue', 'QualifyingHousingRatio',
    'Branch', 'FICO', 'PrimaryBorrowerZip', 'DTI', 'PropertyCity',
    'TotalMortgageLoanAmount', 'CLTV', 'PrimaryBorrowerCity',
    'SentToProcessingDate', 'Investor', 'Region', 'InterestRate', 'LoanPurpose',     'LTV', 'LockDate'
]

date_feats = ['DocSentDate',
...
loan_purpose_options = ['Purchase', 'Cash-Out Refi', 'No Cash-Out Refi', 'None', 'Other']
...
for index, field in enumerate(fields):
    ...
    with cols[col_index]:
        ...
        elif field == 'LoanPurpose':
            input_data[field] = st.selectbox(field, loan_purpose_options)
        elif field == 'Region':
...
# setting the minimum date for the date time features
    minimum_dates = {
    ...
    # creating a dataframe from the input data, using the names as the columns
    df = pd.DataFrame(input_data, index=[0])
    df_ = df.copy()

    # importing the pipeline and the model
    pipeline = joblib.load('preprocessor.pkl')
    base_model = joblib.load('base_model.pkl')

    # doing necessary transformations before preprocessing
    datetime_features = ['DocSentDate', 'InitialDisclosureDate', 'ApplicationReceivedDate', 'DocOrderedDate', 'CreateDate', 'LockExpirationDate', 'SentToProcessingDate', 'LockDate', 'ReadyforDocs']
    for feature in datetime_features:
        df[feature] = pd.to_datetime(df[feature])
        # now subtracting the present datetime from the minimum datetime for each feature
        df[feature] = (df[feature] - pd.to_datetime(minimum_dates[feature])).dt.days


    # now using the pipeline to preprocess the data
    X = pipeline.transform(df)

    # now using the model to predict the funding date
    y_pred = base_model.predict(X)

    # showing the output in streamlit and rounding to a whole number
    st.write('The predicted funding days is: ', round(y_pred[0]))

    # to predict the expected funding date, we will need to add the predicted funding days to the InitialDisclosureDate
    days = round(y_pred[0])
    # making sure that the InitialDisclosureDate is in datetime format
    df_['InitialDisclosureDate'] = pd.to_datetime(df_['InitialDisclosureDate'])
    # now adding the days to the InitialDisclosureDate
    expected_funding_date = df_['InitialDisclosureDate'] + pd.to_timedelta(days, unit='d')
    # showing the expected funding date in streamlit
    st.write('The expected funding date is: ', expected_funding_date[0].date())
